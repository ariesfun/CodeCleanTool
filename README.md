...
![扫描结果：名称列用图标区分目录与文件，类型列按分类着色，右侧为文件详情与瘦身统计](docs/image/screenshot-main-new.jpg)
...
- **规则管理** — 清理/保留两区段分组，自定义增删、拖拽排序（同步至匹配优先级）、导入导出；裸扩展名自动补全（`.exe` → `*.exe`）
...
22 个独立探针、493 条断言，覆盖规则匹配、扫描、清理、打包、配置、模型排序等模块；不启动完整 GUI 即可验证业务逻辑。
